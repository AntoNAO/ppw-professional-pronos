...
add column if not exists prediction_options text[];

update public.matches
set prediction_options = '{}'::text[]
where prediction_options is null;

alter table public.matches
alter column prediction_options set default '{}'::text[];
...
begin
  ...
  if exists (
    with payload as (
      select
        (item->>'match_id')::uuid as match_id,
        trim(coalesce(item->>'prediction', '')) as prediction
      from jsonb_array_elements(predictions_input) item
    )
    select 1
    from payload
    join public.matches m
      on m.id = payload.match_id
     and m.event_id = event_id_input
    where cardinality(coalesce(m.prediction_options, '{}'::text[])) > 0
      and not exists (
        select 1
        from unnest(m.prediction_options) option_value
        where lower(trim(option_value)) = lower(trim(payload.prediction))
      )
  ) then
    raise exception 'INVALID_PREDICTION_OPTION';
  end if;
